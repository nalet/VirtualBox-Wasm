  ...
  function execBlockWrapped(cpuP, ramB, maxInsn) {
    ...
    if (now - statLastReport > 5e3) {
      statLastReport = now;
      {
        ...
        const sorted = [ ...fallbackOpcodes.entries() ].sort((a, b) => b[1] - a[1]).slice(0, 8);
        const topStr = sorted.map(([op, cnt]) => "0x" + op.toString(16) + ":" + cnt).join(" ");
        console.log("[JIT] calls=" + statTotalCalls + " insns=" + statTotalInsns + " fallbacks=" + statFallbacks + " avg=" + (statTotalInsns / Math.max(1, statTotalCalls - statFallbacks)).toFixed(1) + " @" + statLastCSIP + " top=[" + topStr + "]");
      }
    }
    return n;
  }
