  ...
  function execBlock(cpuP, ramB, maxInsn) {
    ...
    // Log JIT calls at CS=0003 (ISOLINUX) to trace execution before crash
    {
      ...
      if (curCS <= 16) {
        ...
        if (cnt <= 300) console.log("[JIT-ISOL] #" + cnt + " CS=" + curCS.toString(16) + " IP=" + ip.toString(16).padStart(4, "0") + " exec=" + executed + " bail=" + (lastBailOp >= 0 ? "0x" + lastBailOp.toString(16) : "none") + " code=" + cb + " AX=0x" + gr16(0).toString(16).padStart(4, "0") + " DX=0x" + gr16(2).toString(16).padStart(4, "0"));
      }
    }
    return executed;
  }
